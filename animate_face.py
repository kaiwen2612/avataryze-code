...
class FaceAnimationClass:
    ...
    def _conver_input_frame(self, frame):
        ...
        # return torch.tensor(frame[np.newaxis]).permute(0, 3, 1, 2).cpu()
...
def animate_face(username, driverfile, imgfile):
    from tqdm import tqdm
    import time
    faceanimation = FaceAnimationClass(source_image_path=imgfile, use_sr=False)

    capture = cv2.VideoCapture(driverfile)
    ...
    writer = imageio.get_writer(os.path.join("static", "output", "{}.mp4".format(username)), fps=fps, quality=9, macro_block_size=1, 
